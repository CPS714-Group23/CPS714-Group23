...
const menuItemsMiddle = [
  {name: 'Scheduler', icon: <Calendar />, path: '/2'},
  {name: 'Drug supply tracker', icon: <PrescriptionBottle />, path: '/3'},
  {name: 'Prescription', icon: <RxFile />, path: '/4'},
  {name: 'Drug interaction checker', icon: <Analytics />, path: '/5'},
    ...
    <Drawer variant="permanent" open={open} className='side-panel'>
      ...
      <List style={{ marginTop: '6em' }}>
          ...
          <ListItem key={item.name} component={Link} to={item.path} disablePadding
            ...
            sx={{ display: 'block'}}
            style={{
              color: location.pathname === item.path ? '#7B9B69' : '#FFF',
              backgroundColor: location.pathname === item.path ? '#FFF' : '#7B9B69',
              borderRadius: '20px',
            }}>
              <ListItemButton
              sx={{
